...
static void print_rx_bds(ec_priv_t * priv)
{
    ...
    printk(KERN_DEBUG"---- rx ring status ----\n");
    printk(KERN_DEBUG"rx_bd_base = 0x%p\n", priv->rx_bd_base);
    printk(KERN_DEBUG"cur_rx = 0x%p\n", priv->cur_rx);

    printk(KERN_DEBUG"---- rx bds ----\n");
    printk(KERN_DEBUG"     status\t control\t buf addr\n");
...
static inline void set_mac_addr(ec_priv_t * ecp)
{
    if (ecp->mac_addr) {
        ...
        //printk(KERN_DEBUG"using previous one\n");
        return;
    }
    
    if ('?' != macaddr[0]) {
        //printk(KERN_DEBUG"parse mannual mac address\n");
    ...
    //printk(KERN_DEBUG"set default mac address \n");
...
//            printk("NOT interested status: 0x%08x\n", (u32)status);
//            break;
//        }
...
static int ec_netdev_init(struct net_device *dev)
{
    ...
    INFO_GREEN(KERN_INFO"net device : %s init over.\n", dev->name);
  #if 0
    if (ecp->phy_model == ETH_PHY_MODEL_ATC2605) {
        hw_regs->er_opmode &= ~(EC_OPMODE_SR | EC_OPMODE_ST);  // stop tx and rx
        hw_regs->er_ienable = 0;
...
static int __init asoc_ethernet_probe(struct platform_device *pdev)
{
	...
	ecp->phy_irq = ASOC_ETHERNET_PHY_IRQ;
#endif
