...
static void isp_capture_start(struct owl_camera_dev *cam_dev)
{
    ...
    if (V4L2_MBUS_CSI2 == cam_param->bus_type) { 
		ret |= owl_camera_hw_call(hw_adapter, mipi_csi_init, icd);
    }
...
static void isp_capture_stop(struct owl_camera_dev *cam_dev)
{
    struct soc_camera_device *icd = cam_dev->icd;
    struct owl_camera_param *cam_param = icd->host_priv;
    unsigned long flags,isp_int_stat;
    int ret;
	...
	ret |= owl_camera_hw_call(hw_adapter, clear_channel_preline_int_en, cam_param->channel);
	isp_int_stat = owl_camera_hw_call(hw_adapter, get_channel_state,cam_param->channel);
	ret |= owl_camera_hw_call(hw_adapter, clear_channel_preline_pending, isp_int_stat,cam_param->channel);
...
static irqreturn_t owl_camera_host_isp_isr(int irq, void *data)
{
   ...
   // struct v4l2_subdev *sd = soc_camera_to_subdev(icd);
	...
	DBG_INFO("Enter isr");
	reset = 0;
		preline_int_pd = hw_adapter->preline_int_pd[cam_param->channel];
		frameend_int_pd = hw_adapter->frameend_int_pd[cam_param->channel];
	spin_lock_irqsave(&cam_dev->lock, flags);
	isp_int_stat = owl_camera_hw_call(hw_adapter, get_channel_state,cam_param->channel);
	...
	{
	ret = owl_camera_hw_call(hw_adapter, clear_channel_frameend_pending,isp_int_stat,cam_param->channel);
	}
...
static int owl_camera_add_device(struct soc_camera_device *icd)
{
	...
	struct clk *csi_clk = cam_dev->csi_clk;
    ...
    printk(KERN_INFO "isp clock is %dM", (int) (clk_get_rate(isp_clk) / 1000000));
...
//		act_writel((act_readl(0xb01600a0)|(0x1<<13)),0xb01600a0);
	    ...
	    err = clk_enable(csi_clk);
	    if (err) {
	        DBG_ERR("enable csi clock failed %d", err);
	        return err;
	    }
	    printk(KERN_INFO "the csi clock : %d,0xb01600a0 : 0x%x", mipi_cfg->csi_clk,act_readl(0xb01600a0));
	    err = clk_set_rate(csi_clk, mipi_cfg->csi_clk);
	    if (err) {
	        DBG_ERR("set csi clock rate failed %d", err);
	        return err;
	    }
	    printk(KERN_INFO "csi clock is %dM", (int) (clk_get_rate(csi_clk) / 1000000));
...
static inline void sensor_pwd_info_init(struct sensor_pwd_info *spinfo)
{
    ...
    spinfo->rear_gpio_reset.num = -1;
	spinfo->front_gpio_reset.num = -1;
...
static int camera_clock_init(struct owl_camera_dev *cdev)
{
	...
	if (IS_ERR(csi_parent_clk)) {
        ...
        DBG_ERR("get csi_parent_clk clock error%d", err);
    }
	
	clk_set_parent(cdev->csi_clk, csi_parent_clk);
...
static int reset_gpio_init(struct device_node *fdt_node, struct sensor_pwd_info *spinfo, int rf_flag)
{
  ...
  if(SENSOR_FRONT == rf_flag){
	  ...
	  DBG_INFO("%s: num-%d, active-%s", r_gpio_name, r_gpio->num, r_gpio->active_level ? "high" : "low");
	  DBG_INFO("%s: num-%d, active-%s", f_gpio_name, f_gpio->num, f_gpio->active_level ? "high" : "low");
...
static int owl_camera_remove(struct platform_device *pdev)
{
	...
	owl_camera_hw_call(hw_adapter, hw_adapter_exit,pdev);
