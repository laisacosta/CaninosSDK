...
static int camera_module_g_mbus_config(struct v4l2_subdev *sd, struct v4l2_mbus_config *cfg)
{
	...
	if(SENSOR_FLAG_DVP == (desc->flags&SENSOR_FLAG_INTF_MASK)){
...
static int camera_module_reset(struct device *dev)
{
    ...
    int rear_cam = !!g_sensor_cfg.rear;    
    owl_isp_reset(dev,rear_cam,host_id);
    return 0;
}
                            ...
                            struct i2c_driver *idrv)
{
    ...
    printk("install as [%s] camera\n", (dsc->rear ? "REAR" : "FRONT"));
    printk("i2c adapter[%d], host[%d], channel[%d], bus[%s], output data[%s]\n", 
