...
static bool my_disable_irq = false;

static int nfc_of_data_get(struct bcm2079x_dev *bcm2079x_dev){
        struct device_node *of_node;
        enum of_gpio_flags flags;

        of_node = of_find_compatible_node(NULL, NULL, "bcm2079x");
            if (of_node==NULL){
                printk(KERN_ERR"%s,%d,find the bcm2079x dts err!\n",__func__, __LINE__);
            return -1;
        }

	if (!of_find_property(of_node, "irq_gpio", NULL)) {
		printk("<nfc>err: no config irq_gpio\n");
		return -1;
	}
	bcm2079x_dev->irq_gpio = of_get_named_gpio_flags(of_node, "irq_gpio", 0, &flags);

	if (!of_find_property(of_node, "en_gpio", NULL)) {
		printk("<nfc>err: no config en_gpio\n");
		return -1;
	}
	bcm2079x_dev->en_gpio = of_get_named_gpio_flags(of_node, "en_gpio", 0, &flags);

	if (!of_find_property(of_node, "wake_gpio", NULL)) {
		printk("<nfc>err: no config wake_gpio\n");
		return -1;
	}
	bcm2079x_dev->wake_gpio = of_get_named_gpio_flags(of_node, "wake_gpio", 0, &flags);

	printk("<MH>==================irq_gpio=%d en_gpio=%d wake_gpio=%d \n",
                       bcm2079x_dev->irq_gpio,bcm2079x_dev->en_gpio,bcm2079x_dev->wake_gpio);
...
{
	struct bcm2079x_dev *bcm2079x_dev = filp->private_data;//i2c_get_clientdata(client);
...
#ifdef USE_SGPIO
	gpio_value = gpio_get_value_cansleep(bcm2079x_dev->irq_gpio);
#else
        gpio_value = gpio_get_value(bcm2079x_dev->irq_gpio);
#endif
	
        spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
	
	if(!gpio_value && (bcm2079x_dev->count_irq < 1) )
	{
		spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
	...
	}else
	{
		...
		spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
		printk(KERN_INFO"<Mh>poll there is data to read!!! no wait any more.\n");
...
static void bcm2079x_dev_suspend(struct early_suspend *dev)
{
        struct bcm2079x_dev *bcm2079x_dev = container_of(dev, struct bcm2079x_dev, early_suspend);
...
/*
        ...
        while(wake_lock_active(&bcm2079x_dev->wake_lock ))
        {
            printk("[bcm2079x suspend] release wake lock!!!\n");
            wake_unlock(&bcm2079x_dev->wake_lock);
        }
#endif
*/
...
static void bcm2079x_dev_resume(struct early_suspend *dev)
{
        struct bcm2079x_dev *bcm2079x_dev = container_of(dev, struct bcm2079x_dev, early_suspend);
#ifdef USE_SGPIO
        gpio_set_value_cansleep(bcm2079x_dev->irq_gpio, 0);
			   ...
			   const struct i2c_device_id *id)
{
	...
	ret = nfc_of_data_get(bcm2079x_dev);
        if(ret != 0){
                printk(KERN_ERR"%s,%d,Pls check bcm2079x dts !!!\n",__func__, __LINE__);
                goto err_gpio;
        }
	ret = gpio_request(bcm2079x_dev->irq_gpio, "nfc_int");
	if (ret){
		printk("<MH> %sgpio_request irq_gpio nfc_int\n",__func__);
		goto err_irq;
	}
	ret = gpio_request(bcm2079x_dev->en_gpio, "nfc_ven");
	if (ret){
		printk("<MH> %sgpio_request irq_gpio nfc_ven\n",__func__);
		goto err_en;
	}
	ret = gpio_request(bcm2079x_dev->wake_gpio, "nfc_firm");
	if (ret){
		printk("<MH> %sgpio_request irq_gpio nfc_firm\n",__func__);
		goto err_firm;
	}


	gpio_direction_output(bcm2079x_dev->en_gpio, 0 );
	gpio_direction_output(bcm2079x_dev->wake_gpio, 1);
       
#ifdef USE_SGPIO 
	gpio_set_value_cansleep(bcm2079x_dev->irq_gpio, 0);
#else
        gpio_set_value(bcm2079x_dev->irq_gpio, 0);
#endif
	gpio_direction_input(bcm2079x_dev->irq_gpio);
	
	client->irq =gpio_to_irq(bcm2079x_dev->irq_gpio);
	if(client->irq == -ENXIO) {
	 ...
	 */
	dev_info(&client->dev, "requesting IRQ %d with IRQF_NO_SUSPEND NR_OWL_PHY_IRQS = %d\n", client->irq,NR_OWL_PHY_IRQS);
	bcm2079x_dev->irq_enabled = true;  

#ifdef USE_SGPIO
	ret = request_threaded_irq(client->irq, NULL, bcm2079x_dev_irq_handler,
			  IRQF_TRIGGER_RISING, "bcm2079x", bcm2079x_dev);
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	bcm2079x_dev->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 2;
	bcm2079x_dev->early_suspend.suspend = bcm2079x_dev_suspend;
	bcm2079x_dev->early_suspend.resume = bcm2079x_dev_resume;
	register_early_suspend(&bcm2079x_dev->early_suspend);
#endif
	dev_info(&client->dev,
...
/*                           AP6441                                */
/*******************************************************************/
static const unsigned short normal_i2c[] = {0x76, I2C_CLIENT_END};//0x76

static struct of_device_id bcm2079x_of_match[] = {
	{ .compatible = "bcm2079x" },
	{ }
};
static struct i2c_driver bcm2079x_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "bcm2079x-i2c",
		.of_match_table	= of_match_ptr(bcm2079x_of_match),
	},
