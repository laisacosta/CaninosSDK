...
void owl_isp_reset(struct device *dev, int rear, int host_id)
{
    struct sensor_pwd_info *pi = to_spinfo(host_id);
    struct dts_gpio *reset= NULL;
    if(rear)
    	reset = &pi->rear_gpio_reset;
    else
    	reset = &pi->front_gpio_reset;    	
	printk(" rear=%d  reset_pin:%d  %s() %d\n",rear,reset->num,__FUNCTION__,__LINE__);
	/* RESET */
    ...
    struct dts_sensor_config *dsc, const char *name)
{
	...
	if(dsc->rear){
	    if (of_property_read_u32(fdt_node, "rear_channel", &dsc->channel)) {
	        printk(KERN_ERR "err: fail to get sensor channel\n");
	        goto fail;
	    }

	    if (of_property_read_string(fdt_node, "rear_bus_type", &bus_type)) {
	        printk(KERN_ERR "err: faild to get sensor bus type\n");
	        goto fail;
	    }
		if (of_property_read_string(fdt_node, "rear_data_type", &data_type)) {
	        printk(KERN_ERR "err: faild to get sensor output data type\n");
	        goto fail;
	    }
		
	}else{
	    if (of_property_read_u32(fdt_node, "front_channel", &dsc->channel)) {
	        printk(KERN_ERR "err: fail to get sensor channel\n");
	        goto fail;
	    }

	    if (of_property_read_string(fdt_node, "front_bus_type", &bus_type)) {
	        printk(KERN_ERR "err: faild to get sensor bus type\n");
	        goto fail;
	    }
		if (of_property_read_string(fdt_node, "front_data_type", &data_type)) {
	        printk(KERN_ERR "err: faild to get sensor output data type\n");
	        goto fail;
	    }	
	}
