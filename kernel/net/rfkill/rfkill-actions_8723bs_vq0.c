...
static int acts_wlan_bt_ref_count=0;
...
/*Author:albertliu 设为-1保证在第一次调用rt_bt_power_on
或者rt_bt_power_off都会执行电源控制*/
static int bt_8723as_powered_on = -1;
...
{
        if (bt_8723as_powered_on != 1)
        {    
            bt_8723as_powered_on = 1;
            printk(KERN_INFO MODULE_TAG "bt_8723as_powered_on mt_bt_power_on ++ %d\n", bt_8723as_powered_on);
        ...
        {
            printk(KERN_INFO MODULE_TAG "bt_8723as_powered_on test mt_bt_power_on pass %d\n", bt_8723as_powered_on);            
        }
	return 0;
}
...
{
    if (bt_8723as_powered_on != 0)
    {
        printk(KERN_INFO MODULE_TAG "bt_8723as_powered_on test mt_bt_power_off ++ %d\n", bt_8723as_powered_on);
    ...
    {
        printk(KERN_INFO MODULE_TAG "bt_8723as_powered_on test mt_bt_power_off pass %d\n", bt_8723as_powered_on);        
    }
}
