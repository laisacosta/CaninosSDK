...
void *hdmi_get_mode(struct hdmi_video_settings *settings)
{
    ...
    while (index >= 0) {
		HDMI_DRV_PRINT("settings->vid %d hdmi_display_modes[index].mode.vid %d \n",settings->vid,hdmi_display_modes[index].vid);
    ...
    if(*bootable != 0)
    {
    	...
    	*bit_invert = fdtdec_get_int(gd->fdt_blob, dev_node, "bit_invert", 0);
	    ...
	    if (valide_vid(vid)){
	    	...
	    	* bootvid = -1; 
	        HDMI_DRV_PRINT("%s: not support %s ,we used default vid  = %d\n",__func__, resolution, bootvid);
	    }
    }
    return ret;
}
...
int hdmi_init(void)
{
    ...
    if (fdtdec_get_hdmi_par(&bootable,&bootrotate,&bootvid,&channel_invert,&bit_invert,&i2cb)) {
       HDMI_DRV_PRINT("%s: error, fdtdec_get_hdmi_par: fdt No hdmi par, and now do nothing temply\n", __func__);
    }
    ...
    if(bootable != 0 && hdmi_get_plug_state() == 1 && (bootrotate == 0 || bootrotate == 3)){   
	    ...
	    if (valide_vid(bootvid)){
	        ...
	        HDMI_DRV_PRINT("%s: read vid  success = %d\n",__func__, bootvid);
	    } else {
	        HDMI_DRV_PRINT("%s: first time used config vid  = %d\n",__func__, psink_info->v_settings.vid );
	    }
		...
		HDMI_DRV_PRINT("hdmi_init bootargs %s \n",buf);
